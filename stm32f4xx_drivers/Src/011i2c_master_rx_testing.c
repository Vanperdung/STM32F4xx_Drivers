...
int main(void)
{
    ...
    while(1)
    {
        if(!(button.pGPIOx->IDR & (1 << GPIO_PIN_5)))
        {
            ...
            if(!(button.pGPIOx->IDR & (1 << GPIO_PIN_5)))
            {
                I2C_MasterSendData(&i2c1, &leng_send, 1, SlaveAddr, I2C_ENABLE_SR);
                I2C_MasterReceiveData(&i2c1, &leng, 1, SlaveAddr, I2C_ENABLE_SR);
                I2C_MasterSendData(&i2c1, &data_send, 1, SlaveAddr, I2C_ENABLE_SR);
                I2C_MasterReceiveData(&i2c1, &data, (uint32_t)leng, SlaveAddr, I2C_DISABLE_SR);
            }
